...
sub process_food {
	...
	$vars->{fangan}{others}{name} = "其他";
	my @notes = split/\n/,$vd_food->{$language}{agesuggestion}{$sex}{$ageKey}{note};
	my $note_index = 0;
	foreach my $note (@notes) {
		...
		#push @{$vars->{fangan}{others}{desc}{item}}, {desc => $note};
	}
	$vars->{fangan}{others}{noteindex} = $note_index;
...
sub get_item {
	...
	$item{per100} = "0.1" if ($Data->{per}*100 < 1);
	$item{per100} = "99.9" if ($item{per100} >= 100);
...
sub calculate_level_per {
	...
	foreach my $meta (keys %{$Data}) {
		...
		## get per and level
		($Data->{$meta}{per},$Data->{$meta}{level})=&get_per($Data->{$meta},$meta,$weigth,$level_set);
	}
...
sub USAGE {#
    ...
    Usage:
      ...
      -in         <dir>   product set database list and xml dir     optional
                          default "$Bin/db2xml/20170622";
      -referdir   <dir>   reference pop data dir                    optional
                          default "$Bin/../../reference_data/qpcr_current";
