...
my ($xmlfile,$templateDir,$dirout,$config,$barcode,$sim);
GetOptions (
	"xml=s"		=> \$xmlfile,
...
#### final pdf
if ($sim) {
	&make_finalPDF($templateDir,$dirout,$config,$barcode,"sim");
}
else {
	&make_finalPDF($templateDir,$dirout,$config,$barcode,"all");
}
...
sub make_finalPDF {
	my ($templateDir,$dirout,$config,$barcode,$key)=@_;
	...
	$cmd .= "-mulu $templateDir/mulu.tt" if ($key eq "all");
	$cmd .= "-mulu $templateDir/mulusim.tt" if ($key eq "sim");
	print "$cmd\n";
	`$cmd`;
