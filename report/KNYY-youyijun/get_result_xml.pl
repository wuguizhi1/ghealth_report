...
sub main_process {
	...
	## get prebiotics and probiotics products list
	my @micro_preprobiotics_products = ();

	## 菌群分布(有益菌属分布)
	# $listH->{"distribution"}; $listH->{"pathogens"}
	&Intestinal_Flora($Data,$listH->{distribution},$vars,"$indir/XML/distribution/$language",$language,\@micro_preprobiotics_products);
	...
	&Program_products($vars,"$indir/XML/advise",$language,$samInfo,$listH->{advise},\@micro_preprobiotics_products);
...
#&Program_products($vars,"$indir/XML",$language,$samInfo,$listH->{diseaseList},$micro_preprobiotics_products);
sub Program_products {
	my ($vars,$dir,$language,$samInfo,$list,$micro_preprobiotics_products)=@_;
	my $ageKey=&get_program_age($samInfo->{age});
	my $diseaseclassKey=&get_diseaseclass_key($samInfo->{"疾病分类"});
	## process probiotics and prebiotics products
	&process_biotics_products($vars,$dir,$ageKey,$diseaseclassKey,$samInfo->{sex},$language,$micro_preprobiotics_products);
	
	return;
}

#&process_biotics_products($vars,$dir,$ageKey,$diseaseclassKey,$sex,$language,$micro_preprobiotics_products);
sub process_biotics_products {
	my ($vars,$dir,$ageKey,$diseaseclassKey,$sex,$language,$micro_preprobiotics_products)=@_;

	$vars->{chanpin}{productnote} = "-";

	## micobiome risk related products
	my %products_comp;
	my %products_index;
	my $products_risk_i = 0;
	my ($duration_max, $duration_min) = (0,0);
	if (@{$micro_preprobiotics_products}) {
		$products_risk_i = 1;
		foreach my $product (@{$micro_preprobiotics_products}) {
			my ($product_name, $product_f, $product_d, $product_m, $product_du, $product_mc, $product_warn) = split/：/,$product;
			#print "$product_name\n$product_f\n$product_d\n$product_m\n$product_du\n$product_mc\n";
			if ($product_du=~/(\d+)-(\d+)/) {
				$duration_min = $1 if ($1 > $duration_min);
				$duration_max = $2 if ($duration_max ne "+" && $duration_max < $2);
			}
			if ($product_du=~/(\d+)\+/) {
				$duration_min = $1 if ($1 > $duration_min);
				$duration_max = "+";
			}
			if (!exists $products_comp{$product_name}) {
				$product_name=~/([a-zA-Z0-9]+)$/;
				$products_comp{$product_name}{"enname"} = $1;
				$products_comp{$product_name}{"name"} = $product_name;
				$products_comp{$product_name}{"freq"} = $product_f;
				$products_comp{$product_name}{"dosage"} = $product_d;
				$products_comp{$product_name}{"meal"} = $product_m;
				$products_comp{$product_name}{"duration"} = $product_du;
				$products_comp{$product_name}{"monthconsume"} = $product_mc;
				if (defined $product_warn && $product_warn ne "") {
					$products_comp{$product_name}{"warn"} = $product_warn;
				}
				else {
					$products_comp{$product_name}{"warn"} = "-";
				}

				$products_index{$products_risk_i} = $product_name;
			}
			elsif ($product_mc > $products_comp{$product_name}{"monthconsume"}) {
				$products_comp{$product_name}{"freq"} = $product_f;
				$products_comp{$product_name}{"dosage"} = $product_d;
				$products_comp{$product_name}{"meal"} = $product_m;
				$products_comp{$product_name}{"duration"} = $product_du;
	...
	## 
	my $vd=XMLin("$dir/$language/tjsjcchanpintuijian.xml",NoAttr=>1,SuppressEmpty => "");

	if ($diseaseclassKey ne "-") {
		my $products_desc = $vd->{$language}{dieasesuggestion}{$diseaseclassKey}{$ageKey}{action};
		$vars->{chanpin}{productnote} = $vd->{$language}{dieasesuggestion}{$diseaseclassKey}{$ageKey}{note} if ($vd->{$language}{dieasesuggestion}{$diseaseclassKey}{$ageKey}{note} ne "");

		my @descs = split/\n/,$products_desc;
		foreach my $desc (@descs) {
			next if ($desc !~/^产品/);
			$desc =~/[^：]+：(.*)/;
			my ($product_name, $product_f, $product_d, $product_m, $product_du, $product_mc, $product_warn) = split/：/,$1;
			#print "$product_name\n$product_f\n$product_d\n$product_m\n$product_du\n$product_mc\n";
			if ($product_du=~/(\d+)-(\d+)/) {
				$duration_min = $1 if ($1 > $duration_min);
				$duration_max = $2 if ($duration_max ne "+" && $duration_max < $2);
	...
	if ($products_risk_i == 0) {
		$products_risk_i = 1;
		my $products_desc = $vd->{$language}{agesuggestion}{desc}{$ageKey}{action};
		$vars->{chanpin}{productnote} = $vd->{$language}{agesuggestion}{desc}{$ageKey}{note} if ($vd->{$language}{agesuggestion}{desc}{$ageKey}{note} ne "");
	...
	$vars->{chanpin}{name} = $vd->{$language}{title};
	$vars->{chanpin}{durationmin} = $duration_min;
	$vars->{chanpin}{durationmax} = $duration_max;

	my $item_i = 1;
	foreach my $product_i (sort {$a <=> $b} keys %products_index) {
		my %product_item;
		next if ($products_comp{$products_index{$product_i}}{"enname"} eq "U1" && $diseaseclassKey eq "disease0");
		$products_comp{$products_index{$product_i}}{"index"} = $item_i;
		push @{$vars->{chanpin}{desc}{item}}, \%{$products_comp{$products_index{$product_i}}};
		$vars->{chanpin}{productsnumber} = $item_i;
		$item_i ++;
	}
...
#($diseaseclassKey)=&get_diseaseclass_key($samInfo->{"疾病分类"});
sub get_diseaseclass_key {
	my ($num)=@_;
	my $diseaseclassKey="-";
	...
	if ($num >= 1 && $num <= 7) {
		my $d_num = $num - 1;
		$diseaseclassKey="disease".$d_num;
	}

	return ($diseaseclassKey);
}
...
sub process_prebiotics {
	...
	if ($vars->{changdaojunqun}{fenbu}{prebiotics_sup} eq '-') {
		...
		$prebiotics_sup{num} = 1;
		$prebiotics_sup{desc} = "建议您补充"."$vars->{changdaojunqun}{fenbu}{prebiotics_sup}"."等益生元，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，增加有益物质的产生，有利于增强肠道屏障功能，促进肠道健康。";
		$vars->{fangan}{prebiotics}{suggest1} = "建议您补充"."$vars->{changdaojunqun}{fenbu}{prebiotics_sup}"."等益生元，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，增加有益物质的产生，有利于增强肠道屏障功能，促进肠道健康。";
	}
...
sub process_probiotics {
	...
	if ($vars->{changdaojunqun}{fenbu}{probiotics_sup} eq '-') {
		...
		$probiotics_sup{num} = 1;
		$probiotics_sup{desc} = "建议您补充"."$vars->{changdaojunqun}{fenbu}{probiotics_sup}"."等益生菌产品，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，抑制有害菌的异常增殖，调节肠道菌群平衡。";
		$vars->{fangan}{probiotics}{suggest1} = "建议您补充"."$vars->{changdaojunqun}{fenbu}{probiotics_sup}"."等益生菌产品，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，抑制有害菌的异常增殖，调节肠道菌群平衡。";
	}
...
sub process_food {
	...
	if ($vars->{changdaojunqun}{fenbu}{food_sup} eq '-') {
		...
		$food_sup{num} = 1;
		$food_sup{desc} = "建议您在日常饮食中多选择"."$vars->{changdaojunqun}{fenbu}{food_sup}"."等食物，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，增强肠道健康保护力。";
		$vars->{fangan}{food}{suggest1} = "建议您在日常饮食中多选择"."$vars->{changdaojunqun}{fenbu}{food_sup}"."等食物，促进"."$vars->{changdaojunqun}{fenbu}{biotics_bad}"."等有益菌的生长，增强肠道健康保护力。";
	}
...
sub process_good_badDesc {
	...
	if (@{$good}) {
		if (scalar @{$good} <= 3) {
			...
			$good_desc=join("；",$good->[0],$good->[1],$good->[2]);
			$good_desc.="等";
		}
	}
	...
	if (@{$bad}) {
		if (scalar @{$bad} <= 3) {
			...
			$bad_desc=join("；",$bad->[0],$bad->[1],$bad->[2]);
			$bad_desc.="等";
		}
	}
...
sub item_Micro_Overvwiew {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		my ($add,$addN,$addG)=&get_good_bad_array($item,\@good,\@nobad,\@bad);
...
sub Intestinal_Flora {
	my ($Data,$list,$vars,$dir,$language,$micro_preprobiotics_products)=@_;
	...
	## 分布
	&item_Intestinal_Flora('fenbu',$Data,$list,$vars,$dir,$language,$micro_preprobiotics_products);
...
sub item_Intestinal_Flora {
	my ($prefix,$Data,$list,$vars,$dir,$language,$micro_preprobiotics_products)=@_;
	
	##
	my (@good,@nobad,@bad,@gooditem,@nobaditem,@baditem);
	my (@goodDesc,@badDesc,@goodDisease,@goodSource,@badDisease,@goodEffect,@badEffect,@badSource);
	my (@biotics_good,@biotics_bad);
	my (@probiotics_sup_all,@probiotics_sup_bad1,@probiotics_sup_bad2,@probiotics_sup_bad3);
	my (@prebiotics_sup_all,@prebiotics_sup_bad1,@prebiotics_sup_bad2,@prebiotics_sup_bad3);
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		if (defined $item->{risk} && $item->{risk} eq "有害") {
			...
			&process_PreProbiotics_and_food($vars->{age},\@prebiotics_sup_all,\@prebiotics_sup_bad1,\@prebiotics_sup_bad2,\@prebiotics_sup_bad3,\@probiotics_sup_all,\@probiotics_sup_bad1,\@probiotics_sup_bad2,\@probiotics_sup_bad3,\@food_sup_all,\@food_sup_bad1,\@food_sup_bad2,\@food_sup_bad3,$vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc}) if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} ne "");
			&process_PreProbiotics_products($vars->{age},$micro_preprobiotics_products,$vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc}) if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} ne "");
		}
		else {
			push @nobaditem,$item;
		}
	...
	my @prebiotics_sup;
	@prebiotics_sup = (@prebiotics_sup_bad3, @prebiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} <= 2);
	@prebiotics_sup = (@prebiotics_sup_bad2, @prebiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} == 3);
	@prebiotics_sup = (@prebiotics_sup_bad1, @prebiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} == 4);
	my $prebiotics_sup_uniq = &uniq_array(\@prebiotics_sup_bad2);
	#my $prebiotics_sup_uniq = &uniq_array(\@prebiotics_sup);

	my @probiotics_sup;
	@probiotics_sup = (@probiotics_sup_bad3, @probiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} <= 2);
	@probiotics_sup = (@probiotics_sup_bad2, @probiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} == 3);
	@probiotics_sup = (@probiotics_sup_bad1, @probiotics_sup_all) if ($vars->{changdaojunqun}{$prefix}{badN} == 4);
	my $probiotics_sup_uniq = &uniq_array(\@probiotics_sup_bad2);
	...
	my $food_sup_uniq = &uniq_array(\@food_sup_bad2);
	#my $food_sup_uniq = &uniq_array(\@food_sup);

	##
	push @{$vars->{changdaojunqun}{$prefix}{abnormalitem}},@baditem,@gooditem;
	## good,bad
	($vars->{changdaojunqun}{$prefix}{good},$vars->{changdaojunqun}{$prefix}{nobad},$vars->{changdaojunqun}{$prefix}{bad})=&process_good_bad2(\@good,\@nobad,\@bad);
...
sub process_PreProbiotics_and_food {
	my ($age,$probiotics_sup_all,$probiotics_sup_bad1,$probiotics_sup_bad2,$probiotics_sup_bad3,$prebiotics_sup_all,$prebiotics_sup_bad1,$prebiotics_sup_bad2,$prebiotics_sup_bad3,$food_sup_all,$food_sup_bad1,$food_sup_bad2,$food_sup_bad3,$Actdesc)=@_;
		...
		if ($Act =~ /(\d+)\\textasciitilde (\d+)）：/) {
			if ($age >= $1 && $age <= $2) {
				if ($Act =~/^益生元/) {
					$Act=~/^[^：]+：(.*)/;
					my @sups=split/；/,$1;
					unshift @{$probiotics_sup_all},@sups;
					my $bad1 = &randomElem ('1', \@sups);
					push @{$probiotics_sup_bad1}, @{$bad1};
					my $bad2 = &randomElem ('2', \@sups);
					push @{$probiotics_sup_bad2}, @{$bad2};
					my $bad3 = &randomElem ('3', \@sups);
					push @{$probiotics_sup_bad3}, @{$bad3};
				}
				if ($Act =~/^益生菌/) {
					$Act=~/^[^：]+：(.*)/;
					my @sups=split/；/,$1;
					unshift @{$prebiotics_sup_all},@sups;
					my $bad1 = &randomElem ('1', \@sups);
					push @{$prebiotics_sup_bad1}, @{$bad1};
					my $bad2 = &randomElem ('2', \@sups);
					push @{$prebiotics_sup_bad2}, @{$bad2};
					my $bad3 = &randomElem ('3', \@sups);
					push @{$prebiotics_sup_bad3}, @{$bad3};
				}
				if ($Act =~/^食物/) {
					$Act=~/^[^：]+：(.*)/;
					my @sups=split/；/,$1;
...
# &process_PreProbiotics_products($vars->{age},$micro_preprobiotics_products,$vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc}) if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{actiondesc} ne "");
sub process_PreProbiotics_products {
	my ($age,$micro_preprobiotics_products,$Actdesc)=@_;

	my @Actarray = split/\n/,$Actdesc;

	foreach my $Act (@Actarray) {
		$Act =~s/^\s+|\s+$//;
		next if ($Act eq "" or $Act !~/^产品/);
		if ($Act =~ /(\d+)\\textasciitilde (\d+)）：/) {
			if ($age >= $1 && $age <= $2) {
				$Act=~/^[^：]+：(.*)/;
				my @sups=split/；/,$1;
				push @{$micro_preprobiotics_products}, @sups;
			}
		}
		elsif ($Act =~ /(\d+)\+）：/) {
			if ($age >= $1) {
...
sub get_item {
	...
	if ($Data->{level} ne '-') {
		...
		if ($vd->{$language}{suggestion}{healthy}{$Data->{level}}{mechanism} ne "") {
			...
			$item{riskdescfull}=$vd->{$language}{suggestion}{healthy}{$Data->{level}}{mechanismfull} if ($vd->{$language}{suggestion}{healthy}{$Data->{level}}{mechanismfull} ne "");
		}
		else {
			$item{riskdesc}='-';
		}
...
sub get_good_bad_array {
	...
	if (! defined $item->{risk}) {
		...
		if ($item->{risk} ne "正常") {
			$add=1;
			if ($item->{risk} eq "有益") {
				if (defined $item->{cnname}) {
					push @{$nobad},$item->{cnname};
					push @{$good},$item->{cnname};
				}
				else {
					push @{$nobad},$item->{name};
					push @{$good},$item->{name};
				}
				$addG=1;
			}
...
#($vars->{yingyanggongneng}{desc}{$prefix}{good},$vars->{yingyanggongneng}{desc}{$prefix}{bad})=&process_good_bad(\@good,\@bad);
sub process_good_bad2 {
	my ($goodA,$nobadA,$badA)=@_;
	my ($good,$nobad,$bad)=('-','-','-');
	...
	if (@{$nobadA}) {
		if (scalar @{$nobadA} <= 5) {
			...
			$nobad=join("、",$nobadA->[0],$nobadA->[1],$nobadA->[2],$nobadA->[3],$nobadA->[4]);
			#$nobad.="等";
		}
	}
	##
...
sub load_sample_info {
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@head;$i++) {
			if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "weight" || $head[$i] eq "height" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate") 
			{
			...
			elsif ($head[$i] eq "name" || $head[$i] eq "clincID" || $head[$i] eq "department" || $head[$i] eq "doctor") {
				...
				if ($head[$i] =~ /疾病分类/) {
					if (defined $unit[$i] && ($unit[$i]>=1 && $unit[$i]<=7)) {
						$samInfo{"疾病分类"} = $unit[$i];
					}
					else {
						$samInfo{"疾病分类"} = "0";
