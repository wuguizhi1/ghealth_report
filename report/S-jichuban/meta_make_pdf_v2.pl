...
my ($xmlfile,$templateDir,$dirout,$config,$barcode,$docconfig);
GetOptions (
	"xml=s"		=> \$xmlfile,
	"template=s"	=> \$templateDir,
	"config=s"	=> \$config,
	"docconfig=s"	=> \$docconfig,
	"od=s"		=> \$dirout,
	"barcode=s"	=> \$barcode,
);
...
$config=$config || "$Bin/config.all";
$config=&AbsolutePath("file",$config);
$docconfig=$docconfig || "$Bin/config.doc";
$docconfig=&AbsolutePath("file",$docconfig);
$templateDir=$templateDir || "$Bin/template/current";
...
my (@context1);#,,@context3,@context4,@context5);
my (@context2);
#### 扉页
...
&make_pdf($xml,"gaikuangP1",$templateDir,$dirout,\@context1);
&make_pdf($xml,"gaikuangP2",$templateDir,$dirout,\@context1);
&make_pdf($xml,"fenbuP1",$templateDir,$dirout,\@context1);
&make_pdf($xml,"fenbuP2",$templateDir,$dirout,\@context1);
&make_pdf($xml,"yingyanggongneng",$templateDir,$dirout,\@context1);
&make_pdf($xml,"jianyi",$templateDir,$dirout,\@context1);

#### 医生简版报告 
&make_pdf($xml,"abnormalitem",$templateDir,$dirout,\@context2);
&make_pdf($xml,"jianyi",$templateDir,$dirout,\@context2);

#### output context
&output_contexts("$dirout/contexts1",\@context1);
&output_contexts("$dirout/contexts2",\@context2);

#foreach my $pageinfo (@context2) {
#	if ($pageinfo->{key} eq "abnormalitem.pdf") {
#		if ($pageinfo->{pageNum}%2 eq 0) {
#			$docconfig=$docconfig || "$Bin/config.evennum.doc";
#		}
#		else {
#			$docconfig=$docconfig || "$Bin/config.oddnum.doc";
#		}
#	}
#}
#$docconfig=&AbsolutePath("file",$docconfig);
...
my $doc_prefix = "$barcode"."_doc";
&make_finalPDF($templateDir,$dirout,$docconfig,$doc_prefix);
