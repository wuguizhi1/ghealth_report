...
sub get_program_regulate {
	...
	if ($vars->{gaikuang}{desc}{duoyangxing}{risk} eq "有害" && $vars->{gaikuang}{desc}{duoyangxing}{level} eq "l1") {
		$condition="condition1";
	}
	elsif ($vars->{gaikuang}{desc}{youyijun}{risk} eq "有害" && $vars->{gaikuang}{desc}{youyijun}{level} eq "l1") {
		$condition="condition2";
	}
	elsif ($vars->{gaikuang}{desc}{youhaijun}{risk} eq "有害") {
...
sub get_program_food {
	...
	if ($disease eq '-') {
		...
		elsif (($vars->{changdaojunqun}{pathogen}{badN} >= 3) && $condition eq '-') {
			$condition="condition4";
		}
		return ($condition) if ($condition ne '-');
	}
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@head;$i++) {
			if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "age" || $head[$i] eq "weight" || $head[$i] eq "height" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate") 
			{
				...
				$unit[$i]="e源基因" if ($unit[$i]=~/广州西陇生物技术有限公司/);
				$samInfo{$head[$i]}=$unit[$i];
				$vars->{$head[$i]}=$unit[$i];
			}
