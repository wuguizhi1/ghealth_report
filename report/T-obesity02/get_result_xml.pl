...
sub Compare_history {
	...
	if (@historys) {
		for (1..@historys) {
			...
			$vars->{$batch_str}{changdaojunqun}{pangjun}{goodN}=$xml->{changdaojunqun}{pangjun}{goodN};
			$vars->{$batch_str}{changdaojunqun}{pangjun}{badN}=$xml->{changdaojunqun}{pangjun}{badN};
			$vars->{$batch_str}{changdaojunqun}{pangjun}{good}=$xml->{changdaojunqun}{pangjun}{good};
			$vars->{$batch_str}{changdaojunqun}{pangjun}{bad}=$xml->{changdaojunqun}{pangjun}{bad};
			$vars->{$batch_str}{changdaojunqun}{shoujun}{goodN}=$xml->{changdaojunqun}{shoujun}{goodN};
			$vars->{$batch_str}{changdaojunqun}{shoujun}{badN}=$xml->{changdaojunqun}{shoujun}{badN};
			$vars->{$batch_str}{changdaojunqun}{shoujun}{good}=$xml->{changdaojunqun}{shoujun}{good};
			$vars->{$batch_str}{changdaojunqun}{shoujun}{bad}=$xml->{changdaojunqun}{shoujun}{bad};
		}
	}
}
...
sub Program {
	...
	&process_food($vars,$dir,$ageKey,$samInfo->{sex},$samInfo->{disease},$language);
...
sub process_regulate {
	...
	my $regulate_condition = &get_regulate_condition($vars);

	$vars->{fangan}{regulate}{desc} = $vd->{$language}{bmisuggestion}{$regulate_condition}{$ageKey}{action};
	$vars->{fangan}{regulate}{note} = $vd->{$language}{bmisuggestion}{$regulate_condition}{$ageKey}{note};
...
sub get_regulate_condition {
	my ($vars) = @_;
	my $regulate_condition = "bmi3";

	my ($pangjun_index, $shoujun_index) = (0,0);
	my $pangjun_badper = $vars->{changdaojunqun}{pangjun}{badN}/$vars->{changdaojunqun}{pangjun}{totalN};
	my $shoujun_badper = $vars->{changdaojunqun}{shoujun}{badN}/$vars->{changdaojunqun}{shoujun}{totalN};
	if ($vars->{changdaojunqun}{pangjun}{badN} >= 3) {
		$pangjun_index = 1;
	}
	if ($vars->{changdaojunqun}{shoujun}{badN} >= 5) {
		$shoujun_index = 1;
	}

	if ($pangjun_index == 1 && $shoujun_index == 1) {
		if ($pangjun_badper >= $shoujun_badper) {
			$regulate_condition = "bmi1";
		}
		else {
			$regulate_condition = "bmi2";
		}
	}
	elsif ($pangjun_index == 1) {
		$regulate_condition = "bmi1";
	}
	elsif ($shoujun_index == 1) {
		$regulate_condition = "bmi2";
	}
	elsif ($vars->{gaikuang}{desc}{duoyangxing}{risk} eq "有害") {
		$regulate_condition = "bmi3";
	}
	else {
		$regulate_condition = "bmi4";
	}

	return($regulate_condition);
}
...
sub process_food {
	my ($vars,$dir,$ageKey,$sex,$disease,$language)=@_;
	
	## food
	if ($disease->{'高血压'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme1.pdf";
	}
	elsif ($vars->{BMI} >= 24 && $disease->{'高血脂'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme2.pdf";
	}
	elsif ($vars->{BMI} >= 24 && $disease->{'三餐不规律'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme3.pdf";
	}
	elsif ($vars->{BMI} >= 24 && $disease->{'肉食居多'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme4.pdf";
	}
	elsif ($vars->{BMI} >= 24 && $sex eq "M") {
		$vars->{fangan}{food}{desc} = "food_scheme5.pdf";
	}
	elsif ($vars->{BMI} >= 24 && $sex eq "F") {
		$vars->{fangan}{food}{desc} = "food_scheme6.pdf";
	}
	elsif ($vars->{BMI} < 24 && $disease->{'高血脂'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme7.pdf";
	}
	elsif ($vars->{BMI} < 24 && $disease->{'三餐不规律'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme8.pdf";
	}
	elsif ($vars->{BMI} < 24 && $disease->{'肉食居多'}{'risk'} eq "Y") {
		$vars->{fangan}{food}{desc} = "food_scheme9.pdf";
	}
	elsif ($vars->{BMI} < 24 && $vars->{age} > 45) {
		$vars->{fangan}{food}{desc} = "food_scheme10.pdf";
	}
	elsif ($vars->{BMI} < 24 && $sex eq "M") {
		$vars->{fangan}{food}{desc} = "food_scheme11.pdf";
	}
	elsif ($vars->{BMI} < 24 && $sex eq "F") {
		$vars->{fangan}{food}{desc} = "food_scheme12.pdf";
	}
	...
	if ($disease->{'高血压'}{'risk'} eq "Y") {
		$vars->{fangan}{sport}{desc} = "sport_scheme1.pdf";
	}
	elsif ($vars->{BMI} >= 24) {
		$vars->{fangan}{sport}{desc} = "sport_scheme2.pdf";
	}
	elsif ($vars->{BMI} < 24) {
		$vars->{fangan}{sport}{desc} = "sport_scheme3.pdf";
	}
...
sub get_item {
	...
	if ($key eq "tjsjcjunqunduoyangxing") {
		...
		$item{valuelocation}=sprintf "%.4f",(5*$item{value}/6+0.52);
	}
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@head;$i++) {
			if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "weight" || $head[$i] eq "height" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate") 
			{
				...
				elsif ($head[$i] eq "日常饮食习惯") {
					$unit[$i] = decode("UTF-8",$unit[$i]);
					$samInfo{disease}{$unit[$i]}{'risk'}="Y";
					if ($unit[$i] =~/肉食居多/) {
						$samInfo{disease}{"肉食居多"}{'risk'}="Y";
					}
					else {
						$samInfo{disease}{"肉食居多"}{'risk'}="N";
					}
				}
				elsif ($head[$i] eq "心脑血管疾病") {
					$unit[$i] = decode("UTF-8",$unit[$i]);
					if ($unit[$i] eq "无" || $unit[$i] eq "" || $unit[$i] eq "N") {
						$samInfo{disease}{$head[$i]}{'risk'}="N";
						$samInfo{disease}{"高血压"}{'risk'}="N";
						$samInfo{disease}{"高血脂"}{'risk'}="N";
					}
					else {
						$samInfo{disease}{$head[$i]}{'risk'}="Y";
					}

					if ($unit[$i]=~/高血压/ && $unit[$i]=~/高血脂/) {
						$samInfo{disease}{"高血脂"}{'risk'}="Y";
						$samInfo{disease}{"高血压"}{'risk'}="Y";
					}
					elsif ($unit[$i]=~/高血压/) {
						$samInfo{disease}{"高血压"}{'risk'}="Y";
						$samInfo{disease}{"高血脂"}{'risk'}="N";
...
sub get_per {
	...
	if ($key ne "tjsjcjunqunduoyangxing") {
