...
###
my ($datadir,$indir,$dirout,$language,$barcode,$level_set,$help);
GetOptions (
	"data=s"	=> \$datadir,
	"in=s"		=> \$indir,
	"out=s"		=> \$dirout,
...
my ($Data)=&calculate_meta_level_per($datadir,$indir,$barcode,$term2oriid,$level_set);

## main process
#&main_process($Data,$listH,\%{$diseaseOrder{question}},\%{$diseaseOrder{predict}},\%vars,$datadir,$indir,$language,$barcode,$samInfo);
&main_process($Data,$listH,\%{$diseaseOrder{question}},\%{$diseaseOrder{predict}},\%{$diseaseOrder{names}},\%vars,$datadir,$indir,$language,$barcode,$samInfo);
...
sub main_process {
	my ($Data,$listH,$listQ,$listP,$listN,$vars,$datadir,$indir,$language,$barcode,$samInfo)=@_;
...
sub Program {
	...
	&process_food_sport($vars,$dir,$foodKey,$sportKey,$language);
	## process regulate
...
sub process_regulate {
	my ($vars,$dir,$regulateKey,$regulate,$language)=@_;
	...
	if ($regulateKey=~/condition/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{microsuggestion}->{$regulateKey}->{action}};
	}
	elsif ($regulateKey=~/disease/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{dieasesuggestion}->{$regulateKey}->{action}};
	}
	elsif ($regulateKey=~/age/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{agesuggestion}->{$regulateKey}->{action}};
	}
...
sub process_food_sport {
	my ($vars,$dir,$foodKey,$sportKey,$language)=@_;
	...
	$vars->{fangan}{food}{desc} = $vd_food->{$language}{dieasesuggestion}{$foodKey}{action} if ($foodKey=~/disease/);
	$vars->{fangan}{food}{desc} = $vd_food->{$language}{microsuggestion}{$foodKey}{action} if ($foodKey=~/condition/);
	$vars->{fangan}{food}{desc} = $vd_food->{$language}{agesuggestion}{$foodKey}{action} if ($foodKey=~/age/);
	## sport
	...
	$vars->{fangan}{sport}{desc} = $vd_sport->{$language}{dieasesuggestion}{$sportKey}{action} if ($sportKey=~/disease/);
	$vars->{fangan}{sport}{desc} = $vd_sport->{$language}{microsuggestion}{$sportKey}{action} if ($sportKey=~/condition/);
	$vars->{fangan}{sport}{desc} = $vd_sport->{$language}{agesuggestion}{$sportKey}{action} if ($sportKey=~/age/);
...
sub get_program_regulate {
	...
	if ($vars->{gaikuang}{desc}{duoyangxing}{risk} eq "有害") {
		$condition="condition1";
	}
	elsif ($vars->{gaikuang}{desc}{youyijun}{risk} eq "有害" && $condition eq '-') {
		$condition="condition2";
	}
	elsif ($vars->{gaikuang}{desc}{youhaijun}{risk} eq "有害" && $condition eq '-') {
		$condition="condition3";
	}
	elsif (($vars->{changdaojunqun}{pathogen}{badN} >= 1 || $vars->{changdaojunqun}{pathogen}{num} >= 5) && $condition eq '-') {
...
sub item_Micro_Metabolism {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}) {
		...
		if (defined $item->{risk} && $item->{risk} eq "有害") {
			...
			push @badDisease,$vd->{$language}{suggestion}{$item->{level}}{aiddisease} if (defined $vd->{$language}{suggestion}{$item->{level}}{aiddisease} && $vd->{$language}{suggestion}{$item->{level}}{aiddisease} ne "");
			push @badEffect,$vd->{$language}{suggestion}{$item->{level}}{effectdesc} if (defined $vd->{$language}{suggestion}{$item->{level}}{effectdesc} && $vd->{$language}{suggestion}{$item->{level}}{effectdesc} ne "");
			push @badActRule,$vd->{$language}{suggestion}{$item->{level}}{actionrule} if (defined $vd->{$language}{suggestion}{$item->{level}}{actionrule} && $vd->{$language}{suggestion}{$item->{level}}{actionrule} ne "");
			push @badActDesc,$vd->{$language}{suggestion}{$item->{level}}{actiondesc} if (defined $vd->{$language}{suggestion}{$item->{level}}{actiondesc} && $vd->{$language}{suggestion}{$item->{level}}{actiondesc} ne "");
			## get badDesc
			push @badDesc,$vd->{$language}{suggestion}{$item->{level}}{mechanism} if (defined $vd->{$language}{suggestion}{$item->{level}}{mechanism} && $vd->{$language}{suggestion}{$item->{level}}{mechanism} ne "");
		...
		if (defined $item->{risk} && $item->{risk} eq "有益") {
			### Get risk disease info
			push @goodDesc,$vd->{$language}{suggestion}{$item->{level}}{mechanism} if (defined $vd->{$language}{suggestion}{$item->{level}}{mechanism} && $vd->{$language}{suggestion}{$item->{level}}{mechanism} ne "");
			push @goodDisease,$vd->{$language}{suggestion}{$item->{level}}{aiddisease} if (defined $vd->{$language}{suggestion}{$item->{level}}{aiddisease} && $vd->{$language}{suggestion}{$item->{level}}{aiddisease} ne "");
			push @goodEffect,$vd->{$language}{suggestion}{$item->{level}}{effectdesc} if (defined $vd->{$language}{suggestion}{$item->{level}}{effectdesc} && $vd->{$language}{suggestion}{$item->{level}}{effectdesc} ne "");
		}
...
#				push @{$regulate},{name => $vd->{$language}{suggestion}{$item->{level}}{actionrule}, desc => $vd->{$language}{suggestion}{$item->{level}}{actiondesc}};
			}
		}
...
sub item_Micro_Summary {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		$vars->{gaikuang}{$prefix}{summary}=$vd->{$language}{suggestion}{$lev}{part1};
	}
...
sub get_item {
	...
	if ($key eq "tjsjcyouyijun" || $key eq "tjsjcyouhaijun") {
		...
		$item{per100} = "1.00" if ($Data->{per}*100 < 1 && $key eq "tjsjcyouyijun");
	...
	if ($Data->{level} ne '-') {
		$item{risk}=$vd->{$language}->{suggestion}->{$Data->{level}}->{effect};
		if ($vd->{$language}->{suggestion}->{$Data->{level}}->{mechanism} ne "") {
			$item{riskdesc}=$vd->{$language}->{suggestion}->{$Data->{level}}->{mechanism};
		}
		else {
			$item{riskdesc}='-';
		}
...
sub check_file {
	...
	#
	$file="$indir/reference_population/refer.extract.xls";
	$check+=&checkF($file);
	#
	$file="$indir/reference_population/refer.extract.stat.xls";
	$check+=&checkF($file);
	
	## check config file
	#
	$file="$indir/reference_population/config/genus_latin2zh";
	$check+=&checkF($file);
	#
	$file="$indir/reference_population/config/genus_fenbu.conf";
	$check+=&checkF($file);
	#
	$file="$indir/reference_population/config/Vitamin.conf";
	$check+=&checkF($file);
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@head;$i++) {
			if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "age" || $head[$i] eq "weight" || $head[$i] eq "height" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate") 
			{
				...
				if ($head[$i] eq "粪便质地" || $head[$i] eq "粪便颜色") {
...
sub calculate_meta_level_per {
	my ($datadir,$indir,$barcode,$term2oriid,$level_set)=@_;
	my $Data=();
...
sub load_sample_reference_data {
	my ($datadir,$indir,$barcode,$term2oriid)=@_;
	my %Data=();
