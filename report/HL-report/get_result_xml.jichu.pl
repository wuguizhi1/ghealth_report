...
sub sample_result {
	...
	$worksheet->write( 5, 0, $vars->{gaikuang}{desc}{duoyangxing}{name} );
	...
	$worksheet->write( 6, 0, $vars->{gaikuang}{desc}{youyijun}{name} );
	...
	$worksheet->write( 7, 0, $vars->{gaikuang}{desc}{youhaijun}{name} );
	...
	$worksheet->write( 10, 0, '肠道菌群相对丰度' );
	$worksheet->write( 11, 0, '检测内容' );
	$worksheet->write( 11, 1, '检测结果' );
	$worksheet->write( 11, 2, '参考范围' );
	...
	foreach my $item (@{$vars->{abundance}{item}}) {
		...
		$worksheet->write( $sheet_row, 1, $item->{value} );
		$worksheet->write( $sheet_row, 2, $item->{threshold} );
		$sheet_row ++;
	}
	my $part2 = "";
	if ($vars->{abundance}{pearson_cor} > 0.92) {
		$part2 .= "您的肠道菌群相对丰度性与参考人群趋势一致，说明您的肠道菌群组成结构及稳定性良好，有利于肠道及人体健康。您的肠道菌群不易受到外界不良因素的干扰，但请避免不良作息、刺激性食物、药物滥用等，这样有助于保持良好的肠道菌群相对丰度，维持肠道微生态健康。";
	}
	elsif ($vars->{abundance}{pearson_cor} > 0.8) {
		$part2 .= "您的肠道菌群相对丰度性与参考人群趋势相似，说明您的肠道菌群组成结构较好，不易受到外界不良因素的干扰，但仍需要遵循良好的饮食及生活习惯，避免不良作息、刺激性食物、药物滥用等，有助于优化肠道菌群组成结构，并保持较好的肠道菌群相对丰度，维持肠道微生态健康。";
	}
	else {
		$part2 .= "您肠道菌群门水平含量与参考人群趋势存在差异，说明您的肠道菌群组成可能引起肠道问题，如腹泻、腹痛、腹胀等。您的肠道菌群较易受到外界不良因素的干扰，需通过饮食调理、按时作息以及合理运动等良好生活方式调理肠道菌群结构，改善肠道微生态健康。";
	}
	$worksheet->write( $sheet_row, 2, $part2 );

	$sheet_row += 2;

	$worksheet->write( $sheet_row, 0, '有益菌检测结果' );
	$sheet_row ++;
	...
	$worksheet->write( $sheet_row, 0, '有害菌检测结果' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	...
	if ($vars->{changdaojunqun}{harmful}{badN} == 0) {
		...
		$part4 .= "综合您的肠道主要有害菌检测结果，$vars->{benificial}{bad}的含量异常，";
		if ($vars->{changdaojunqun}{harmful}{badEffect} ne '-') {
			$part4 .= "不利于$vars->{benificial}{badEffect}，";
		}
		if ($vars->{changdaojunqun}{harmful}{badDisease} ne '-') {
			$part4 .= "甚至增加$vars->{benificial}{badDisease}的风险，";
		}
		$part4 .= "需引起注意。"
	}
	...
	$worksheet->write( $sheet_row, 0, '致病菌检测结果' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	...
	if ($vars->{changdaojunqun}{pathogen}{num} == 0) {
		...
		if ($vars->{changdaojunqun}{pathogen}{badN} == 0) {
			$part5 .= "检测出$vars->{changdaojunqun}{pathogen}{num}种致病菌，含量均在正常范围，尽管这些致病菌含量未超标，当身体免疫力较强时可能不会出现不适感。但请注意，当您的免疫力较弱时，可能引起腹泻、腹痛、感染等病症。建议您持续监测肠道健康，一旦检测出较高含量的致病菌，建议咨询专科医生，酌情做进一步检查。";
		}
		else {
			$part5 .= "检测出$vars->{changdaojunqun}{pathogen}{num}种致病菌，其中$vars->{changdaojunqun}{pathogen}{bad}的含量异常，当身体免疫力较强时可能不会出现不适感。但请注意，当您的免疫力较弱时，初期可能引起腹泻、腹痛、感染等病症，长期可能导致上表中所述疾病。建议您持续监测，必要时请到正规医院咨询专科医生，遵从医生的建议做进一步检查。";
		}
	}
	...
	$worksheet->write( $sheet_row, 0, '粪便样品中肠道菌群营养功能分析' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
...
sub load_sample_info {
	...
	while (<IN>) {
		...
		my @unit=split /\t/,$_;
		for (my $i=0;$i<@head;$i++) {
			if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "age" || $head[$i] eq "weight" || $head[$i] eq "height" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate") 
			{
				$samInfo{$head[$i]}=$unit[$i];
				$vars->{$head[$i]}=$unit[$i] if ($head[$i] eq "barcode" || $head[$i] eq "sex" || $head[$i] eq "age" || $head[$i] eq "batch" || $head[$i] eq "receivedate" || $head[$i] eq "testdate");
			}
