...
#### 潜在疾病风险
&make_pdf($xml,"jibingfengxian",$templateDir,$dirout,\@context2);

#### 调节方案建议
&make_pdf($xml,"tiaojiefangan",$templateDir,$dirout,\@context3);

#### 饮食及运动方案
&get_foodsport_pdf($xml,$templateDir,$dirout);

#### 肠道菌群概况
#&make_pdf($xml,"jiexi",$templateDir,$dirout,\@context2);
&make_pdf($xml,"fenbu",$templateDir,$dirout,\@context4);
&make_pdf($xml,"fenbuP2",$templateDir,$dirout,\@context4);
...
sub read_xml {
	...
	##
	my $xml=XMLin($xmlfile,NoAttr=>1,ForceArray => ['item','baditem','abnormalitem','disease','spot'], KeyAttr => []);
...
#&get_foodsport_pdf($xml,$templateDir,$dirout);
sub get_foodsport_pdf {
	my ($xml,$templateDir,$dirout)=@_;

	mkdir "$dirout/cores" if (! -d "$dirout/cores");
	if (-f "$templateDir/cores/$xml->{fangan}{food}{desc}") {
		`cp $templateDir/cores/$xml->{fangan}{food}{desc} $dirout/cores/shanshifangan.pdf `;
	}
	else {
		print STDERR "Unexists file $templateDir/cores/$xml->{fangan}{food}{desc}, please check...\n";
		die;
	}

	if (-f "$templateDir/cores/$xml->{fangan}{sport}{desc}") {
		`cp $templateDir/cores/$xml->{fangan}{sport}{desc} $dirout/cores/yundongfangan.pdf `;
	}
	else {
		print STDERR "Unexists file $templateDir/cores/$xml->{fangan}{sport}{desc}, please check...\n";
		die;
	}
