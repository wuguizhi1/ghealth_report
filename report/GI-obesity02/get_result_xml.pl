...
sub process_regulate {
	...
	my $regulate_badmax = 0;
	my %regulates_info;
	foreach my $actdesc (@actdescs) {
		$actdesc =~ /(^\d+):(.*)/;
		$regulates_info{$1} = $2;
		$regulate_badmax = $1 if ($1 > $regulate_badmax);
	}

	if (exists $regulates_info{$vars->{metasummary}{desc}{badN}}) {
		my @regulates_item = split/；/, $regulates_info{$vars->{metasummary}{desc}{badN}};
		my $reg_i = 1;
		foreach (@regulates_item) {
			my %regulate_item;
			$_=~s/\[|\]//g;
			my ($item_name, $item_guide, $item_note) = split/：/,$_;
			$regulate_item{name} = $item_name;
			$regulate_item{guide} = $item_guide;
			$regulate_item{note} = "-";
			$regulate_item{note} = $item_note if (defined $item_note);
			$regulate_item{'index'} = $reg_i;
			push @{$vars->{fangan}{regulate}{desc}{item}}, \%regulate_item;
			$reg_i ++;
		}
	}
	elsif ($vars->{metasummary}{desc}{badN} > $regulate_badmax) {
		my @regulates_item = split/；/, $regulates_info{$regulate_badmax};
		my $reg_i = 1;
		foreach (@regulates_item) {
			my %regulate_item;
			$_=~s/\[|\]//g;
...
#	}
#	elsif ($vars->{metasummary}{desc}{badN} > $1 && $reg_i == 0) {
#		
#	}
...
sub item_Micro_Overvwiew {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		## get one item
		my ($item)=&get_item($vd,$Data->{$meta},$language,$meta);
		$item->{markindex} = $item->{per100}*1.615;
...
sub item_Intestinal_Flora {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		## get goodDesc,badDesc
		if (defined $item->{risk} && $item->{risk} eq "有害") {
			$item->{badindex} = $vars->{changdaojunqun}{$prefix}{badN};
			push @baditem,$item;
			push @{$vars->{changdaojunqun}{$prefix}{baditem}},$item;
			### Get risk disease info
