...
### 产品推荐
#&make_front($xml,"chanpintuijian",$templateDir,$dirout);
#&cat_chanpinPDF($dirout,$barcode);
...
sub cat_chanpinPDF {
	...
	my $pdf1 = "$dirout/PDF/chanpintuijian.pdf";
	my $pdf2 = "$dirout/cores/chanpin_intro.pdf";
	if (!-f $pdf1 || !-f $pdf2) {
		print "Unexists $pdf1 or $pdf2 file, please check!";
		exit;
	}

	my $cat_cmd = "pdftk $pdf1 $pdf2 cat output $dirout/$barcode"."_wlyt.pdf";
	system($cat_cmd);
...
#&make_chanpinPDF($templateDir,$dirout,$config,$barcode);
sub make_chanpinPDF {
	my ($templateDir,$dirout,$config,$barcode)=@_;
