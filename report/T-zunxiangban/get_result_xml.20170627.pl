...
sub Program {
	...
	## get disease
	($diseaseF,$diseaseR)=&get_program_disease($samInfo->{disease},$vars->{changdaojunqun}{diseaseHigh},$listQ,$listP);
	## get risk bad disease
	($badDiseaseF,$badDiseaseR)=&get_bad_disease_meta($badDiseaseTotal,$listP);
...
sub get_regulate_condition {
	...
	elsif ($vars->{changdaojunqun}{gaikuang}{harmlevel} eq 'l4' || $vars->{changdaojunqun}{gaikuang}{harmlevel} eq 'l5') {
...
sub process_regulate {
	...
	if ($regulateC ne '-' && $disease ne 'zxchangyan') {
		...
		if ($disease eq '-' && $badDisease eq '-') {
			...
			push @{$vars->{fangan}{regulate}{desc}{item}},{name => $vd->{$language}{regulate}{desc}{$age}{nutrititle}, desc => $vd->{$language}{regulate}{desc}{$age}{nutridesc}, spotnum => 0} if ($vd->{$language}{regulate}{desc}{$age}{nutridesc} ne "");
		}
		elsif ($badDisease ne '-') {
		...
		my $vd=XMLin("$dir/disease/$language/$disease.xml",NoAttr=>1,SuppressEmpty => "");
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => $vd->{$language}{regulate}{desc}{$age}{title}, desc => $vd->{$language}{regulate}{desc}{$age}{desc}, spotnum => 0};
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => $vd->{$language}{regulate}{desc}{$age}{nutrititle}, desc => $vd->{$language}{regulate}{desc}{$age}{nutridesc}, spotnum => 0} if ($vd->{$language}{regulate}{desc}{$age}{nutridesc} ne "");
	}
	elsif ($badDisease ne '-') {
		my $vd=XMLin("$dir/disease/$language/$badDisease.xml",NoAttr=>1,SuppressEmpty => "");
...
#		push @{$vars->{fangan}{regulate}{desc}{item}},{name => $vd->{$language}->{metaregulate}->{$age}->{title}, desc => $desc, spotnum => $spotnum, spot => \@spot};
	...
	if ($vars->{xiaohuahexishou}->{metabolism}->{explain}->{vitaminbadN} > 0) {
...
sub process_food_sport {
	...
	if ($disease ne '-') {
		my $vd=XMLin("$dir/disease/$language/$disease.xml",NoAttr=>1,SuppressEmpty => "");
		&get_food($vd->{$language}{food}{desc}{$age},$vars);
		&get_sport($vd->{$language}{sport}{desc}{$age},$vars);
	}
	elsif ($regulateC ne '-') {
		my $vd=XMLin("$dir/dysbiosis/$language/zxjunqunshihengtiaojie.xml",NoAttr=>1,SuppressEmpty => "");
...
sub get_program_disease {
	my ($diseaseH,$diseaseHigh,$listQ,$listP)=@_;
...
#($badDiseaseF,$badDiseaseR)=&get_bad_disease_meta($badDiseaseTotal,$samInfo->{disease});
