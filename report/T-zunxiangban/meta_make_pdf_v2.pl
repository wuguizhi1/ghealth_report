...
my ($xmlfile,$templateDir,$referdir,$dirout,$config,$barcode);
GetOptions (
	"xml=s"		=> \$xmlfile,
...
&make_pdf($xml,"zongshuoming",$templateDir,$dirout,\@context5);
&make_pdf($xml,"shanshifangan",$templateDir,$dirout,\@context5);
&make_pdf($xml,"changdaotiaojieyuyundongfangan",$templateDir,$dirout,\@context5);
...
sub make_finalPDF {
	...
	##
	my $cmd="perl $Bin/report.pl -conf $config -template $templateDir/pages.tt -outdir $dirout -id $barcode  -tables $dirout -contexts $dirout -mulu $templateDir/mulu.tt -barcode $dirout/$barcode.png ";
	print "$cmd\n";
	`$cmd`;
...
sub USAGE {#
    ...
    Program Function: generate T zunxiangban pdf report;
    Version:    $version
