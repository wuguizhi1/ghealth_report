...
sub main_process {
	...
	## 消化和吸收
	my $goodDiseaseTotal;
	($regulateLow,$goodDiseaseTotal,$badDiseaseTotal)=&Absorption_and_Metabolism($Data,$listH->{absorption},$vars,"$indir/XML/absorption/$language",$language,$Vitamin);

	## 炎症和免疫
	&Inflammation_and_Immune($Data,$listH->{giimmune},$vars,"$indir/XML/giimmune/$language",$language,$regulateLow);
...
sub get_zonglan_pic {
	...
	$p1 ++ if ($vars->{gaikuang}{desc}{badN} >= 1);
	$p1 ++ if ($vars->{changdaojunqun}{fenbu}{badN} >= 3);
	$p1 ++ if ($vars->{changdaojunqun}{pathogen}{badN} >= 3);
	...
	$p2 ++ if ($vars->{xiaohuahexishou}{zongping}{explain}{badN} >= 2);
	$p2 ++ if ($vars->{yingyanggongneng}{desc}{badN} >= 3);
	$p2 ++ if ($vars->{yanzhenghemianyi}{giimmune}{explain}{badN} >= 1);
	if ($p2 == 0) {
...
sub item_Inflammation_and_Immune {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		push @{$vars->{yanzhenghemianyi}{$prefix}{item}},$item;
		...
		if (defined $item->{risk} && $item->{risk} eq "有害") {
			...
			push @{$vars->{yanzhenghemianyi}{$prefix}{baditems}{item}},\%new_item;
			###
		...
		if (defined $item->{risk} && $item->{risk} eq "有益") {
			...
			push @{$vars->{yanzhenghemianyi}{$prefix}{gooditems}{item}},\%new_item;
	...
	($vars->{yanzhenghemianyi}{$prefix}{explain}{goodDisease},$vars->{yanzhenghemianyi}{$prefix}{explain}{badDisease},$vars->{yanzhenghemianyi}{$prefix}{explain}{goodEffect},$vars->{yanzhenghemianyi}{$prefix}{explain}{badEffect})=&process_goodbad_DiseaseEffect(\@goodDisease,\@badDisease,\@goodEffect,\@badEffect);

	return;
}

#my ($regulateLow,$badDiseaseTotal)=&Absorption_and_Metabolism($Data,$listH->{absorption},$vars,"$indir/XML/meta/$language",$language);
sub Absorption_and_Metabolism {
	...
	## 总评
	&item_Digestion_and_Absorption('1','zongping',$Data,$list,$vars,"$dir",$language,\@gooditem,\@baditem,\@goodDesc,\@badDesc,\@goodDisease,\@badDisease,\@badFoodRule,\@badFoodMore,\@badFoodLess,\@goodEffect,\@badEffect,\@badActRule,\@badActMore,\@badActLess,\%goodDiseaseTotal,\%badDiseaseTotal);
...
sub item_Digestion_and_Absorption {
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		## get one item
		my ($item)=&get_item($vd,$Data->{$meta},$language,$meta);
		push @{$vars->{xiaohuahexishou}{$prefix}{item}},$item;
		...
		if (defined $item->{risk} && $item->{risk} eq "有害") {
			###
			#push @{$vars->{xiaohuahexishou}{$prefix}{baditems}{item}},{index => $vars->{xiaohuahexishou}{desc}{$prefix}{badN}, name => $item->{name}, risk => $item->{level}};
			my %new_item = %{$item};
			$new_item{index} = $vars->{xiaohuahexishou}{$prefix}{explain}{badN};
			push @{$vars->{xiaohuahexishou}{$prefix}{baditems}{item}},\%new_item;
			$new_item{index} = $vars->{xiaohuahexishou}{explain}{AbsorbbadN};
			push @{$vars->{xiaohuahexishou}{baditems}{item}},\%new_item if ($prefix eq "protein" || $prefix eq "fat" || $prefix eq "carbon");
			### Get risk disease info
			push @badDesc,$vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} ne "");
			push @badEffect,$vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} ne "");
			push @badDisease,$vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} ne "");
		...
		if (defined $item->{risk} && $item->{risk} eq "有益") {
			...
			push @{$vars->{xiaohuahexishou}{$prefix}{gooditems}{item}},\%new_item;
			$new_item{index} = $vars->{xiaohuahexishou}{explain}{AbsorbbadN};
			push @{$vars->{xiaohuahexishou}{gooditems}{item}},\%new_item if ($prefix eq "protein" || $prefix eq "fat" || $prefix eq "carbon");
			### Get risk disease info
			push @goodDesc,$vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{mechanism} ne "");
			push @goodEffect,$vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{effectdesc} ne "");
			push @goodDisease,$vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} if (defined $vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} && $vd->{$language}{suggestion}{healthy}{$item->{level}}{aiddisease} ne "");
	...
	## Totalgood,Totalbad,TotalgoodDisease,TotalbadDisease,TotalbadFoodRule,TotalbadFoodMore,TotalbadFoodLess,TotalgoodEffect,TotalbadEffect,TotalbadActRule,TotalbadActMore,TotalbadActLess,badDiseaseTotal,goodDiseaseTotal
	## get protein fat carbon risk related effect/disease and actrule/foodrule
	($vars->{xiaohuahexishou}{$prefix}{explain}{goodDisease},$vars->{xiaohuahexishou}{$prefix}{explain}{badDisease},$vars->{xiaohuahexishou}{$prefix}{explain}{goodEffect},$vars->{xiaohuahexishou}{$prefix}{explain}{badEffect})=&process_goodbad_DiseaseEffect(\@goodDisease,\@badDisease,\@goodEffect,\@badEffect);
...
#			my $xmlfile="$historydir/$report_version/$historys[$_].xml";
			if (!-f $xmlfile) {
