...
#### 封面
&make_front($xml,"fengmian",$templateDir,$dirout);
...
#### 营养功能、健康建议
&make_pdf($xml,"xiaohuahexishou",$templateDir,$dirout,\@context1);
&make_pdf($xml,"yingyanggongneng",$templateDir,$dirout,\@context1);
&make_pdf($xml,"yanzhenghemianyi",$templateDir,$dirout,\@context1);

### 疾病风险
#### 潜在疾病风险
&make_pdf($xml,"jibingfengxian",$templateDir,$dirout,\@context2);
&make_pdf($xml,"jianyi",$templateDir,$dirout,\@context3);
...
#### make bar code picture
#`zint -o $dirout/$barcode.raw.png -b 9 --height 42 -d $barcode`;
#`convert $dirout/$barcode.raw.png -colorspace CMYK -compress lossless $dirout/$barcode.png`;
