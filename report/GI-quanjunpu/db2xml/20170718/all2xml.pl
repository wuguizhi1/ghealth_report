	...
	foreach my $itm (@{$pagemodes{$pagemode}{itms}}) {
		foreach my $k (keys %{$itm}) {
			...
			if ($k eq "pic") {
				...
				elsif (($k eq "suggestion" || $k eq "metafood" || $k eq "metaregulate" || $k eq "metasport" || $k eq "agesuggestion" || $k eq "dieasesuggestion" || $k eq "microsuggestion") && exists $pagemodes{$pagemode}{version}) {
