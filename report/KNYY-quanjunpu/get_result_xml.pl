...
###
my ($datadir,$indir,$referdir,$dirout,$testdate,$language,$barcode,$historydir,$confdir,$level_set,$help);
GetOptions (
	"data=s"	=> \$datadir,
	"in=s"		=> \$indir,
	"referdir=s"	=> \$referdir,
...
&process_FBR($datadir,$referdir,\%vars);

## calculate meta level and per by sample data and ref data
my ($Data,$enterotypes)=&calculate_meta_level_per($datadir,$indir,$referdir,$barcode,$term2oriid,$level_set);
#&process_enterotypes_ ($enterotypes,\%vars);
...
#&sample_result(\%vars,$barcode,$dirout);

############################# sub ###############################################
#&get_enterotypes_ ($predominant_genus,\%vars)
sub get_enterotypes_ {
	my ($predominant_genus, $vars) = @_;

	if ($predominant_genus eq "Bacteroides") {
		$vars->{enterotype}{typenum} = 1;
		$vars->{enterotype}{typepic} = "enterotype_1.pdf";
		$vars->{enterotype}{typename} = "拟杆菌型";
	}
	elsif ($predominant_genus eq "Prevotella") {
		$vars->{enterotype}{typenum} = 2;
		$vars->{enterotype}{typepic} = "enterotype_2.pdf";
		$vars->{enterotype}{typename} = "普氏菌型";
	}
	elsif ($predominant_genus eq "Ruminococcus") {
		$vars->{enterotype}{typenum} = 3;
		$vars->{enterotype}{typepic} = "enterotype_3.pdf";
		$vars->{enterotype}{typename} = "瘤胃球菌型";
	}
	else {
		$vars->{enterotype}{typenum} = 4;
		$vars->{enterotype}{typepic} = "enterotype_4.pdf";
		$vars->{enterotype}{typename} = "未分类型";
	}
}

#&process_enterotypes_ ($enterotypes,\%vars)
sub process_enterotypes_ {
	my ($enterotypes,$vars) = @_;

	my $max_genu;

	my @genus = ("Bacteroides","Prevotella","Ruminococcus");
	my @genu_per =sort {$a<=>$b} values %{$enterotypes};
	if ($genu_per[0] == $genu_per[-1]) {
		my $randnum = int(rand(3));
		$max_genu = $genus[$randnum];
	}
	else {
		foreach my $genus (sort {$enterotypes->{$b} <=> $enterotypes->{$a}} keys %{$enterotypes}) {
			$max_genu = $genus;
			last;
		}
	}

	if ($max_genu eq "Bacteroides") {
		$vars->{enterotype}{typenum} = 1;
		$vars->{enterotype}{typepic} = "enterotype_1.pdf";
		$vars->{enterotype}{typename} = "拟杆菌型";
	}
	if ($max_genu eq "Prevotella") {
		$vars->{enterotype}{typenum} = 2;
		$vars->{enterotype}{typepic} = "enterotype_2.pdf";
		$vars->{enterotype}{typename} = "普氏菌型";
	}
	if ($max_genu eq "Ruminococcus") {
		$vars->{enterotype}{typenum} = 3;
		$vars->{enterotype}{typepic} = "enterotype_3.pdf";
		$vars->{enterotype}{typename} = "瘤胃球菌型";
	}
...
sub sample_result {
	...
	my $transfer_HL_result = "-";

	my $outfile="$dirout/$barcode".".result.xlsx";
	#open my $out, '>:encoding(utf-8)', $outfile || die $!;
	open my $fh, '>', $outfile or die "Failed to open filehandle: $!";
	my $workbook  = Excel::Writer::XLSX->new( $fh );
	my $worksheet = $workbook->add_worksheet();
	$worksheet->write( 0, 0, $barcode );
	
	my $head = "5、粪便样品中肠道微生物菌群结构\n";
	$worksheet->write( 1, 0, $head );
	$worksheet->write( 3, 0, '5.1 肠道菌群概况' );
	$worksheet->write( 4, 0, '检测内容' );
	$worksheet->write( 4, 1, '检测结果' );
	$worksheet->write( 4, 2, '检测值' );
	$worksheet->write( 4, 3, '参考范围' );

	$transfer_HL_result = &get_HL_result($vars->{gaikuang}{desc}{duoyangxing}{risk});
	$worksheet->write( 5, 0, "$vars->{gaikuang}{desc}{duoyangxing}{name}($vars->{gaikuang}{desc}{duoyangxing}{latin})" );
	$worksheet->write( 5, 1, $transfer_HL_result );
	$worksheet->write( 5, 2, $vars->{gaikuang}{desc}{duoyangxing}{value} );
	$worksheet->write( 5, 3, $vars->{gaikuang}{desc}{duoyangxing}{threshold} );

	$transfer_HL_result = &get_HL_result($vars->{gaikuang}{desc}{youyijun}{risk});
	$worksheet->write( 6, 0, "$vars->{gaikuang}{desc}{youyijun}{name}($vars->{gaikuang}{desc}{youyijun}{latin})" );
	$worksheet->write( 6, 1, $transfer_HL_result );
	$worksheet->write( 6, 2, $vars->{gaikuang}{desc}{youyijun}{value} );
	$worksheet->write( 6, 3, $vars->{gaikuang}{desc}{youyijun}{threshold} );

	$transfer_HL_result = &get_HL_result($vars->{gaikuang}{desc}{youhaijun}{risk});
	$worksheet->write( 7, 0, "$vars->{gaikuang}{desc}{youhaijun}{name}($vars->{gaikuang}{desc}{youhaijun}{latin})" );
	$worksheet->write( 7, 1, $transfer_HL_result );
	$worksheet->write( 7, 2, $vars->{gaikuang}{desc}{youhaijun}{value} );
	$worksheet->write( 7, 3, $vars->{gaikuang}{desc}{youhaijun}{threshold} );

	
	$worksheet->write( 8, 0, $vars->{gaikuang}{desc}{summary} );

	$worksheet->write( 10, 0, '5.2 肠道菌群相对丰度' );
	$worksheet->write( 11, 0, '检测内容' );
	$worksheet->write( 11, 1, '检测结果' );
	$worksheet->write( 11, 2, '检测值' );
	$worksheet->write( 11, 3, '参考范围' );

	my $sheet_row = 12;
	my $sheet_col = 0;

	foreach my $item (@{$vars->{abundance}{item}}) {
		next if ($item->{value} == 0);
		$worksheet->write( $sheet_row, 0, "$item->{name}($item->{ename})" );
		$worksheet->write( $sheet_row, 1, "$item->{risk}" );
		$worksheet->write( $sheet_row, 2, "$item->{value}%" );
		$worksheet->write( $sheet_row, 3, "$item->{threshold}%" );
		$sheet_row ++;
	}
	$worksheet->write( $sheet_row, 0, "$vars->{FB_ratio}{name}($vars->{FB_ratio}{ename})" );
	$worksheet->write( $sheet_row, 1, $vars->{FB_ratio}{risk} );
	$worksheet->write( $sheet_row, 2, $vars->{FB_ratio}{value} );
	$worksheet->write( $sheet_row, 3, $vars->{FB_ratio}{threshold} );
	$sheet_row ++;

	my $part2 = "";
	if ($vars->{abundance}{pearson_cor} > 0.92) {
		$part2 .= "人体肠道中超过95%的菌群归属于以下四个菌门，拟杆菌门、厚壁菌门、变形菌门和放线菌门，厚壁菌门/拟杆菌门（FBR）比值根据研究表明与肥胖风险正相关。您的肠道菌群相对丰度性与参考人群趋势一致，说明您的肠道菌群组成结构及稳定性良好，有利于肠道及人体健康。您的肠道菌群不易受到外界不良因素的干扰，但请避免不良作息、刺激性食物、药物滥用等，这样有助于保持良好的肠道菌群相对丰度，维持肠道微生态健康。";
	}
	elsif ($vars->{abundance}{pearson_cor} > 0.8) {
		$part2 .= "人体肠道中超过95%的菌群归属于以下四个菌门，拟杆菌门、厚壁菌门、变形菌门和放线菌门，厚壁菌门/拟杆菌门（FBR）比值根据研究表明与肥胖风险正相关。您的肠道菌群相对丰度性与参考人群趋势相似，说明您的肠道菌群组成结构较好，不易受到外界不良因素的干扰，但仍需要遵循良好的饮食及生活习惯，避免不良作息、刺激性食物、药物滥用等，有助于优化肠道菌群组成结构，并保持较好的肠道菌群相对丰度，维持肠道微生态健康。";
	}
	else {
		$part2 .= "人体肠道中超过95%的菌群归属于以下四个菌门，拟杆菌门、厚壁菌门、变形菌门和放线菌门，厚壁菌门/拟杆菌门（FBR）比值根据研究表明与肥胖风险正相关。您肠道菌群门水平含量与参考人群趋势存在差异，说明您的肠道菌群组成可能引起肠道问题，如腹泻、腹痛、腹胀等。您的肠道菌群较易受到外界不良因素的干扰，需通过饮食调理、按时作息以及合理运动等良好生活方式调理肠道菌群结构，改善肠道微生态健康。";
	}
	$worksheet->write( $sheet_row, 0, $part2 );

	$sheet_row += 2;

	$worksheet->write( $sheet_row, 0, '5.3 有益菌检测结果' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	$worksheet->write( $sheet_row, 3, '参考范围' );
	$worksheet->write( $sheet_row, 4, '功能' );
	$sheet_row ++;
	foreach my $item (@{$vars->{changdaojunqun}{benificial}{item}}) {
		$transfer_HL_result = &get_HL_result($item->{risk});
		$worksheet->write( $sheet_row, 0, "$item->{name}($item->{latin})" );
		$worksheet->write( $sheet_row, 1, $transfer_HL_result );
		$worksheet->write( $sheet_row, 2, $item->{value} );
		$worksheet->write( $sheet_row, 3, $item->{threshold} );
		$worksheet->write( $sheet_row, 4, $item->{desc} );
		$sheet_row ++;
	}
	my $part3 = "";
	if ($vars->{changdaojunqun}{benificial}{badN} == 0) {
		$part3 .= "综合您的肠道主要有益菌检测结果，各有益菌含量均处于正常范围，帮助维持肠道健康，有利于产生有益物质、抑制肠道炎症等。肠道菌群含量维持动态平衡，请您保持良好饮食及生活习惯，有助于维持现有的菌群状态。";
	}
	elsif ($vars->{changdaojunqun}{benificial}{badN} < 6) {
		$part3 .= "综合您的肠道主要有益菌检测结果，$vars->{changdaojunqun}{benificial}{bad}的含量异常，";
		if ($vars->{changdaojunqun}{benificial}{badEffect} ne '-') {
			$part3 .= "不利于$vars->{changdaojunqun}{benificial}{badEffect}，";
		}
		if ($vars->{changdaojunqun}{benificial}{badDisease} ne '-') {
			$part3 .= "甚至增加$vars->{changdaojunqun}{benificial}{badDisease}的风险，";
		}
		$part3 .= "需引起注意。";
	}
	$worksheet->write( $sheet_row, 0, $part3 );
	$sheet_row += 2;

	$worksheet->write( $sheet_row, 0, '5.4 有害菌检测结果' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	...
	$worksheet->write( $sheet_row, 4, '功能' );
	$sheet_row ++;
	foreach my $item (@{$vars->{changdaojunqun}{harmful}{item}}) {
		$transfer_HL_result = &get_HL_result($item->{risk});
		$worksheet->write( $sheet_row, 0, "$item->{name}($item->{latin})" );
		$worksheet->write( $sheet_row, 1, $transfer_HL_result );
		$worksheet->write( $sheet_row, 2, $item->{value} );
	...
	my $part4 = "";
	if ($vars->{changdaojunqun}{harmful}{badN} == 0) {
		$part4 .= "综合您的肠道主要有害菌检测结果，各有害菌含量均处于正常范围，有害物质产生少、肠道炎症风险低，整体状况良好。肠道菌群含量维持动态平衡，请您保持良好饮食及生活习惯，有助于维持现有的菌群状态。";
	}
	else {
		$part4 .= "综合您的肠道主要有害菌检测结果，$vars->{changdaojunqun}{harmful}{bad}的含量异常，";
		if ($vars->{changdaojunqun}{harmful}{badEffect} ne '-') {
			$part4 .= "不利于$vars->{changdaojunqun}{harmful}{badEffect}，";
		}
		if ($vars->{changdaojunqun}{harmful}{badDisease} ne '-') {
			$part4 .= "甚至增加$vars->{changdaojunqun}{harmful}{badDisease}的风险，";
		}
		$part4 .= "需引起注意。";
	}
	$worksheet->write( $sheet_row, 0, $part4 );
	$sheet_row += 2;

	$worksheet->write( $sheet_row, 0, '5.5 致病菌检测结果' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	...
	my $part5 = "";
	if ($vars->{changdaojunqun}{pathogen}{num} == 0) {
		$part5 .= "\n本次检测未检出致病菌，未检测到可能导致疾病的致病菌，这是个好消息。当您的免疫力较强时，可持续抑制致病菌的生长。但请注意，当您的免疫力较弱时，肠道致病菌可能会增加，可能引起腹泻、腹痛、感染等病症。";
	}
	else {
		...
		if ($vars->{changdaojunqun}{pathogen}{badN} == 0) {
			$part5 .= "本次检测检出$vars->{changdaojunqun}{pathogen}{num}种致病菌，含量均在正常范围，尽管这些致病菌含量未超标，当身体免疫力较强时可能不会出现不适感。但请注意，当您的免疫力较弱时，可能引起腹泻、腹痛、感染等病症。建议您持续监测肠道健康，一旦检测出较高含量的致病菌，建议咨询专科医生，酌情做进一步检查。";
		}
		else {
			$part5 .= "本次检测检出$vars->{changdaojunqun}{pathogen}{num}种致病菌，其中$vars->{changdaojunqun}{pathogen}{bad}的含量异常，当身体免疫力较强时可能不会出现不适感。但请注意，当您的免疫力较弱时，初期可能引起腹泻、腹痛、感染等病症，长期可能导致上表中所述疾病。建议您持续监测，必要时请到正规医院咨询专科医生，遵从医生的建议做进一步检查。";
		}
	}
	$worksheet->write( $sheet_row, 0, $part5 );
	$sheet_row += 2;

	
	$worksheet->write( $sheet_row, 0, '5.6 粪便样品中肠道菌群营养功能分析' );
	$sheet_row ++;
	$worksheet->write( $sheet_row, 0, '检测内容' );
	$worksheet->write( $sheet_row, 1, '检测结果' );
	$worksheet->write( $sheet_row, 2, '检测值' );
	...
	foreach my $item (@{$vars->{yingyanggongneng}{desc}{item}}) {
		$transfer_HL_result = &get_HL_result($item->{risk});
		$worksheet->write( $sheet_row, 0, $item->{name} );
		$worksheet->write( $sheet_row, 1, $transfer_HL_result );
		$worksheet->write( $sheet_row, 2, $item->{value} );
		$worksheet->write( $sheet_row, 3, $item->{threshold} );
		$worksheet->write( $sheet_row, 4, $item->{desc} );
	...
	#	$part6 .= "综合您的检测结果，您肠道内参与B族维生素和类固醇等营养物质代谢的菌群含量均处于正常范围，帮助您维持机体正常生理功能，请您保持良好饮食及生活习惯，有助于维持现有的菌群状态。";
	#}
	#else {
	#	$part6 .= "综合您的检测结果，您肠道内参与$vars->{yingyanggongneng}{desc}{bad}代谢的菌群含量异常。";
	#	if ($vars->{yingyanggongneng}{desc}{badEffect} ne '-') {
	#		$part6 .= "不利于$vars->{yingyanggongneng}{desc}{badEffect}，";
	#	}
	#	if ($vars->{yingyanggongneng}{desc}{badDisease} ne '-') {
	#		$part6 .= "甚至增加$vars->{yingyanggongneng}{desc}{badDisease}的风险，";
	#	}
	#	$part6 .= "建议您持续监测肠道健康。";
	#}
	#$worksheet->write( $sheet_row, 0, $part6 );
...
sub main_process {
	...
	# summary信息
	my ($summary, $predominant_genus)=&load_summary("$datadir/$barcode.summary.xls");
	&get_enterotypes_ ($predominant_genus, $vars);

	&Intestinal_Flora($Data,$listH->{benificial},"benificial",$vars,"$indir/XML/distribution/$language",$language,$species,$summary);
	&Intestinal_Flora($Data,$listH->{harmful},"harmful",$vars,"$indir/XML/distribution/$language",$language,$species,$summary);
	&Intestinal_Flora($Data,$listH->{distribution},"fenbu",$vars,"$indir/XML/distribution/$language",$language,$species,$summary);
...
sub process_regulate {
	...
	$vars->{fangan}{regulate}{name} = "肠道调节方案";
	#$vars->{fanan}{food}{pic} = "tiaojie.pdf";
	if ($regulateKey=~/condition/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{microsuggestion}->{$regulateKey}->{$age}->{action}};
	}
	elsif ($regulateKey=~/disease/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{dieasesuggestion}->{$regulateKey}->{$age}->{action}};
	}
	elsif ($regulateKey=~/age/) {
		push @{$vars->{fangan}{regulate}{desc}{item}},{name => "益生元和益生菌", desc => $vd->{$language}->{agesuggestion}->{desc}->{$regulateKey}->{action}};
	}
...
sub Micro_Overvwiew {
	...
	## 
	my $diversity_riskdesc = $vars->{gaikuang}{desc}{duoyangxing}{riskdesc};
	my $summary_lev = &process_risk_protect($benifit_risk,$harm_risk);
	&item_Micro_Summary('desc',$diversity_riskdesc,$summary_lev,$list2,$vars,$dir2,$language,$species);
...
sub item_Micro_Summary {
	my ($prefix,$diversity_riskdesc,$lev,$list,$vars,$dir,$language,$species)=@_;
	...
	foreach my $meta (sort {$list->{$a}{order} <=> $list->{$b}{order}} keys %{$list}){
		...
		$summary_info = $vd->{$language}{suggestion}{suggest}{$lev}{part1};
		#$vars->{gaikuang}{$prefix}{summary}=$vd->{$language}{suggestion}{suggest}{$lev}{part1};
	}
	$vars->{gaikuang}{$prefix}{summary}="$diversity_riskdesc"."$summary_info";
...
sub Intestinal_Flora {
	my ($Data,$list,$bacclass,$vars,$dir,$language,$species,$summary)=@_;
	...
	## 分布
	&item_Intestinal_Flora($bacclass,$Data,$list,$vars,$dir,$language,$species);
...
sub get_item {
	...
	$item{plusSD}=$Data->{plusSD};
	$item{minusSD}=$Data->{minusSD};
	$item{level}=$Data->{level};
	$item{range}=$Data->{range};
	$item{latin}=$Data->{latin};
	...
	# 转换有益为正常
	if ($vd->{$language}->{suggestion}->{healthy}->{l1}->{effect} eq "有害") {
		$item{minusSD} = 0.01 if ($item{minusSD} == 0);
		if ($item{value} < $item{minusSD}) {
			$item{level} = "l2";
		}
		$item{threshold} = "≥ $item{minusSD}" ;
		$item{judge} = $item{minusSD};
		$item{level} = "l3" if ($item{level} eq "l4" or $item{level} eq "l5");
	}

	if ($vd->{$language}->{suggestion}->{healthy}->{l5}->{effect} eq "有害") {
		$item{threshold} = "≤ $item{plusSD}";
		$item{judge} = $item{plusSD};
	}

	if ($item{level} ne '-') {
		$item{risk}=$vd->{$language}->{suggestion}->{healthy}->{$item{level}}->{effect};
		if ($vd->{$language}->{suggestion}->{healthy}->{$item{level}}->{mechanism} ne "") {
			$item{riskdesc}=$vd->{$language}->{suggestion}->{healthy}->{$item{level}}->{mechanism};
			$item{riskdescfull}=$vd->{$language}->{suggestion}->{healthy}->{$item{level}}->{mechanismfull} if ($key eq "tjsjcjunqunduoyangxing");
		}
		else {
			$item{riskdesc}='-';
		}
...
sub load_summary {
	...
	$/="\n";
	open IN,$file or die $!;
	my $predominant_genus = "-";
	while (<IN>) {
		chomp;
		next if (/^\#/ || /^$/);
		my ($id,$value)=(split /\s+/,$_)[0,1];
		$summary{$id}=$value;
		$predominant_genus = $value if ($id eq "predominant_genus");
	}
	close IN;
	
	return (\%summary, $predominant_genus);
}
...
sub load_sample_info {
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@head;$i++) {
			...
			elsif ($head[$i] eq "clincID" || $head[$i] eq "department" || $head[$i] eq "doctor" || $head[$i] eq "tester" || $head[$i] eq "auditor")
 			{
				$unit[$i]=decode("UTF-8",$unit[$i]);
				$vars->{$head[$i]}=$unit[$i];
			}
...
#				if ($head[$i] =~ /样本类型/){
#					$head[$i]="samtype";
#					$vars->{$head[$i]}=decode("UTF-8",$unit[$i]);
#				}
...
sub load_list {
	...
	## benificial.list.order$sex
	$file="$indir/benificial.list.order".$samInfo->{sex};
	if (-f $file) {
		&get_list($file,\%listH,\%term2oriid,"benificial");
	}

	## harmful.list.order$sex
	$file="$indir/harmful.list.order".$samInfo->{sex};
	if (-f $file) {
		&get_list($file,\%listH,\%term2oriid,"harmful");
	}
...
sub calculate_meta_level_per {
	...
	($Data,$enterotypes)=&load_sample_reference_data($datadir,$indir,$referdir,$barcode,$term2oriid);

	## get level,per,range
	&calculate_level_per($Data,$level_set);
	
	return ($Data,$enterotypes);
}
...
sub calculate_level_per {
	...
	foreach my $meta (keys %{$Data}) {
		...
		# get level
		($Data->{$meta}{level},$Data->{$meta}{range},$Data->{$meta}{plusSD},$Data->{$meta}{minusSD})=&get_level($Data->{$meta},$meta,$level_set);
...
sub get_level {
	...
	if (exists $meta->{sample} && exists $meta->{ref}) {
		...
		$plusSD=(sprintf "%.2f", ($mean+$sd)*1000);
		$minusSD=(sprintf "%.2f", ($mean-$sd)*1000);
		$minusSD = 0 if ($minusSD < 0);

		$meta->{value} = (sprintf "%.2f", ($meta->{sample})*1000);

		if ($k eq "tjsjcjunqunduoyangxing") {
			$range=(sprintf "%.2f",$mean-2*$sd).'~'.(sprintf "%.2f",$mean+2*$sd);
			$plusSD=(sprintf "%.2f", ($mean+$sd)); 
			$minusSD=(sprintf "%.2f", ($mean-$sd));
			$meta->{value} = (sprintf "%.2f", ($meta->{sample}));
		}
		...
		$level = 'l3';
		if ($meta->{value} < $minusSD) {
			$level = 'l2';
		}
		if ($meta->{value} > $plusSD) {
			$level = 'l4';
		}
	...
	return ($level,$range,$plusSD,$minusSD);
}
...
sub load_sample_reference_data {
	...
	&get_sample_values($file,$term2oriid,\%Data,\%enterotypes);
...
sub get_sample_values {
	my ($file,$term2oriid,$Data,$enterotypes)=@_;
	...
	while (<IN>) {
		...
		if (exists $enterotypes->{$id}) {
			$enterotypes->{$id} = $value;
		}
...
sub process_relative_abundance {
	...
	while (<IN>) {
		chomp;
		next if (/^$/ || /^\#/);
		my ($id,$mean,$sd)=(split /\t/,$_)[0,3,4];
		$hash{$id}{ref}{mean}=$mean;
		$hash{$id}{ref}{sd}=$sd;
		if (!exists $hash{$id}{sam}) {
			$hash{$id}{sam}=0;
		}
	}
	close IN;
...
sub sort_abundance {
	...
	foreach my $key (sort {$hash->{$b}{sam} <=> $hash->{$a}{sam}} keys %{$hash}) {
		...
		$item{per}{control}=$hash->{$key}{ref}{mean};
		push @refer_phylum,$hash->{$key}{ref}{mean};

		$item{value} = sprintf "%.2f",$hash->{$key}{sam}*100;
		my $threshold = sprintf "%.2f",($hash->{$key}{ref}{mean}+$hash->{$key}{ref}{sd})*100;
		$item{risk} = "正常";
		if ($item{value} > $threshold) {
			$item{risk} = "异常";
		}
		$item{threshold} = "≤ $threshold";
		$item{judge} = $threshold;
		#
...
#&process_FBR($datadir,$indir,\%vars);
sub process_FBR {
	my ($datadir,$referdir,$vars)=@_;
	
	##
	my $samFile="$datadir/$vars->{barcode}".".extract.xls";
	my $refFile="$referdir/Firmicutes_Bacteroidetes_ratio.extract.stat.xls";
	## read sample
	$/="\n";

	$vars->{FB_ratio}{name} = "厚壁菌门/拟杆菌门";
	$vars->{FB_ratio}{ename} = "FBR";
	open IN,$samFile or die $!;
	while (<IN>) {
		chomp;
		next if (/^$/ || /^\#/);
		my ($id,$value)=(split /\t/,$_)[0,1];
		next if ($id ne "Firmicutes_Bacteroidetes_ratio");
		$vars->{FB_ratio}{value} = sprintf "%.2f", $value;
	}
	close IN;
	## read ref
	open IN,$refFile or die $!;
	while (<IN>) {
		...
		my ($id,$mean,$sd)=(split /\t/,$_)[0,3,4];
		next if ($id ne "Firmicutes_Bacteroidetes_ratio");
		my $threshold = sprintf "%.2f", ($mean+$sd);
		$vars->{FB_ratio}{threshold} = "≤ $threshold";
		$vars->{FB_ratio}{judge} = $threshold;
		$vars->{FB_ratio}{risk} = "正常";
		$vars->{FB_ratio}{risk} = "异常" if ($vars->{FB_ratio}{value} > $threshold);
...
sub USAGE {#
    ...
    Usage:
      ...
      -out        <dir>   output file dir                           optional
                          default "./";
      -testdate   <str>	  ex:2019-12-12				    forced
