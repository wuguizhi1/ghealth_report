...
$cmd = "perl $Bin/../scripts/sample_fastq2otu_process.pl -l $lable -left $left ";
...
sub clean_clearup_dir_ {
	...
	system "docker run --rm -v $out_dir/:$out_dir/ qiime:1.9.1 rm $out_dir/microbiota/$lable.unmerged_*.fastq";
	system "docker run --rm -v $out_dir/:$out_dir/ qiime:1.9.1 rm $out_dir/microbiota/$lable.merge.stat $out_dir/microbiota/$lable.merge.fastq";
	system "docker run --rm -v $out_dir/:$out_dir/ qiime:1.9.1 rm $out_dir/microbiota/$lable.assign_taxon.sort_L*";

	# clear up function dir
	system "docker run --rm -v $out_dir/:$out_dir/ qiime:1.9.1 rm -rf $out_dir/function/$lable.ko_metagenome_contributions.tab";
	
	system "docker run --rm -v $out_dir/function/:/data ubuntu:14.04 rm -rf /data/uclust_ref_picked_otus/";
}
